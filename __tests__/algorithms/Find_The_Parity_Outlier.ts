import { assert } from 'chai';
import { findOutlier } from './../../src/algorithms/Find_The_Parity_Outlier';

describe('findOutlier', () => {
  it('findOutlier', function () {
    assert.equal(findOutlier([0, 1, 2]), 1);
    assert.equal(findOutlier([1, 2, 3]), 2);
    assert.equal(findOutlier([2, 6, 8, 10, 3]), 3);
    assert.equal(findOutlier([0, 0, 3, 0, 0]), 3);
    assert.equal(findOutlier([1, 1, 0, 1, 1]), 0);
    assert.equal(
      findOutlier([
        75321820, 13944371, 58676797, 23779, -161650741, 43722249, 96809161,
        154145757, 68201113, 105362729, -122588189
      ]),
      75321820
    );
  });
});
